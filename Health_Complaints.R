...
zip.df <- subset(zip.df, ZCTA5CE10 != '40504')
...
data(zipcode)
names <- zipcode
names <- subset(names, (city %in% c('Louisville', 'Prospect', 'Fairdale')) & state == 'KY')

# Plot where the share of A's is highest
ggplot(data=subset(zip.df, Freq != 'NA'), aes(x=long, y=lat, group=group)) +
  geom_polygon(aes(fill = Freq), colour = "white", alpha = 0.5, size =0.2) +
  scale_fill_gradient(low = "#f7fbff",
                      high = "#08306b") +
  geom_path(data = subset(zip.df, Freq != 'NA'), color="gray75", size = 0.2) +
  geom_text(data = names, aes(longitude, latitude, label = zip, map_id = NULL, group = city), size = 2.5) +
  coord_equal()  +
